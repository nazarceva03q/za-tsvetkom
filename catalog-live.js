...
function businessCard(p){return `<article class="product-card business-card">${productPhotos(p)}<div class="product-info"><h3><button data-detail="${p.id}">${esc(p.name)}</button></h3><div><p class="business-photo-price" aria-live="polite">${esc(businessPhotoPrice(p,0))}</p>${p.description?`<p>${esc(p.description)}</p>`:''}</div><p class="product-note">${esc(p.note)}</p><strong class="product-price">${businessPrice(p)}</strong><div class="product-actions"><button class="primary" data-form="${esc(businessRequest(p,0))}">Обсудить оформление</button></div></div></article>`}
